...
export const listOperations = async (request: Request, response: Response) => {
  const { initialDate, finalDate }: { initialDate: string; finalDate: string } =
    request.body;
  try {
    ...
    } else throw new Error("Data Base empty");
  } catch (err: any) {
...
export const deleteAllocantion = async (
  request: Request,
  response: Response
) => {
...
const checkDate = (
  allocations: Array<TypeAllocation>,
  initialDate: string,
  finalDate: string
): Array<TypeAllocation> => {
...
const isDateValid = (
  ...
  if (
    dateYear < parseInt(finalDate.slice(-4)) &&
    dateYear > parseInt(initialDate.slice(-4))
  )
    return true;
  else if (
    dateYear === parseInt(finalDate.slice(-4)) ||
    ...
    if (
      dateMounth < parseInt(finalDate.slice(3, 5)) &&
      dateMounth > parseInt(initialDate.slice(3, 5))
    )
      return true;
    else if (
      dateMounth === parseInt(finalDate.slice(3, 5)) ||
      ...
      if (
        dateDay <= parseInt(finalDate.slice(0, 2)) &&
        dateDay >= parseInt(initialDate.slice(0, 2))
      )
        return true;
  }
